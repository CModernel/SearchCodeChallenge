...
class GetVerticalDetailedSearchResultsUseCaseTest {
    ...
    private lateinit var getVerticalDetailedSearchResultsUseCase: GetVerticalDetailedSearchResultsUseCase

    @Before
    fun setup() {
        getVerticalDetailedSearchResultsUseCase = GetVerticalDetailedSearchResultsUseCase(
    ...
    @Test
    fun `WHEN usecase is executed THEN it fetches vertical detailed search params and emits mapped result`() = runTest {
        ...
        coEvery { mockRequestsRepository.getVerticalDetailedRequestParams() } returns flowOf(mockRequest)
        coEvery { mockResponsesRepository.getVerticalDetailedSectionResults() } returns flowOf(mockResult)
        ...
        getVerticalDetailedSearchResultsUseCase.execute(Unit).test {
            assertEquals(expectedMappedResult, awaitItem())
            awaitComplete()
        }

        // Verify that each dependency is called exactly once
        coVerify(exactly = 1) { mockRequestsRepository.getVerticalDetailedRequestParams() }
        coVerify(exactly = 1) { mockResponsesRepository.getVerticalDetailedSectionResults() }
